...
const DashboardStats = () => {
    ...
    useEffect(() => {
        const fetchStats = async () => {
            setLoading(true);
            if (!activeInstituteId && !isSuperAdmin) {
                setLoading(false);
                return;
            }
            try {
                ...
                const studentsUrl = activeInstituteId
                    ? `${apiBase}/students?institute_id=${activeInstituteId}&status=active`
                    : `${apiBase}/students?status=active`;
                ...
                const employeesUrl = activeInstituteId
                    ? `${apiBase}/employees?institute_id=${activeInstituteId}`
                    : `${apiBase}/employees`;
                ...
                // Fetch courses count
                const coursesUrl = activeInstituteId
                    ? `${apiBase}/courses?institute_id=${activeInstituteId}`
                    : `${apiBase}/courses`;
                ...
                const revenueUrl = activeInstituteId
                    ? `${apiBase}/fee_transactions?institute_id=${activeInstituteId}&payment_date=${today}`
        ...
        fetchStats();
    }, [activeInstituteId, isSuperAdmin]);
